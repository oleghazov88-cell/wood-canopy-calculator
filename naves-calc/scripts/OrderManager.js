...
class OrderManager {
    ...
    async submitOrder(form, calculationData) {
        ...
        // Prepare calculation data with dimensions string if needed
        const calcData = { ...calculationData };
        if (!calcData.dimensions && calcData.length && calcData.width) {
            // Assuming length/width are in meters, convert to mm for formatted string
            calcData.dimensions = `${Math.round(calcData.length * 1000)}x${Math.round(calcData.width * 1000)} мм`;
        }
    ...
    async sendToServer(orderData) {
        try {
            ...
            console.warn('Server unavailable, saving locally as fallback', error);
            // Fallback to local storage if server fails (e.g. offline)
            this.saveOrderLocally(orderData);
            return {
                success: true,
                orderId: `OFFLINE-${Date.now()}`,
                message: 'Сервер недоступен. Заказ сохранен локально.'
            };
        }
    }
