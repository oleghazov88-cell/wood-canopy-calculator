...
class RoofSystem {
    ...
    buildSubstructure(width, length, baseHeight, roofHeight, roofType, material, fullWidth, fullLength) {
        ...
        if (type === 'solid') {
            ...
            const thick = 0.012;
            const osbTex = this.getOSBTexture();
    ...
    getMaterial(type, colorName, mode, roofLength, roofWidth) {
        ...
        if (mode === 'polycarb') {
            ...
            mat = new THREE.MeshStandardMaterial({
                ...
                roughness: 0.4, // Реалистичный полуглянец (было 0.4)
                metalness: 0.6, // Крашеный металл (было 0.6)
                envMapIntensity: 1.0, // Увеличиваем отражения (было 0.9)
                side: THREE.DoubleSide,
                flatShading: false
            });
            ...
            mat = new THREE.MeshStandardMaterial({
                ...
                roughness: 0.4, // Чуть более шершавый чем черепица (было 0.35)
                metalness: 0.6, // (было 0.6 - ок)
                envMapIntensity: 1.0,
                side: THREE.DoubleSide
            });
        } else {
    ...
    getOSBTexture() {
        if (this.textureCache.has('osb-diff')) return this.textureCache.get('osb-diff');
        const size = 512;
        const canvas = document.createElement('canvas');
        canvas.width = size; canvas.height = size;
        const ctx = canvas.getContext('2d');
        ...
        this.textureCache.set('osb-diff', tex);
        return tex;
    }
