...
class CanopyRendererV3 {
    ...
    init3DScene() {
        ...
        if (!isMobile && window.THREE.EffectComposer && window.THREE.SAOPass) {
            ...
            saoPass.params.saoIntensity = 0.05; // Легкая тень, не грязная
    ...
    createGround() {
        ...
        // 3. ЛОГОТИП ЧЕРЕЗ LOGO MANAGER
        if (!this.logoManager) {
            if (typeof LogoManager !== 'undefined') {
                this.logoManager = new LogoManager();
                this.logoManager.init(this.scene);
                this.logoManager.setLogo('assets/textures/logo.svg');
            }
        }
    }
    ...
    createProceduralWoodMaps(colorHex) {
        ...
        for (let i = 0; i < 4000; i++) {
            ...
            ctxD.globalAlpha = 0.05;
            ctxD.fillRect(x, y, 1, 8);

            // Normal speck (roughness noise)
            ctxN.fillStyle = '#9090ff'; // Slight bump
            ctxN.globalAlpha = 0.1;
            ctxN.fillRect(x, y, 1, 8);
        }
    ...
    createWoodMaterial(frameMaterial, frameColoring) {
        // Базовый цвет (если нет текстуры)
        let color = 0xd2b48c; // Светлое дерево (pine)

        // Цвета каркаса (реалистичные оттенки пропиток)
        const frameColors = {
            'no-coloring': 0xE6D0A5, // Натуральная сосна
            'pine': 0xE8C888,        // Сосна (желтоватая)
            'larch': 0xE0BC75,       // Лиственница
            'light-oak': 0xD4B886,   // Светлый дуб
            'oak': 0x967346,         // Дуб
            'dark-oak': 0x634832,    // Темный дуб
            'mahogany': 0x592819,    // Махагон
            'teak': 0x8F6A45,        // Тик
            'walnut': 0x5A4134,      // Орех
            'wenge': 0x362823,       // Венге
            'rosewood': 0x472419,    // Палисандр
            'white': 0xF5F5F0,       // Беленое дерево
            'neomid': 0xD4C179,      // Неомид (примерно)
            'texturol-landscape': 0x8D6E63,
            'texturol-country': 0x4E342E,
            'olsta': 0xA1887F,
            'tikkurila': 0xF0F0F0,
            'gray': 0x757575         // Серый
        };

        // Определяем цвет на основе выбора пользователя
        if (frameColoring && frameColoring !== 'default') {
            if (frameColors[frameColoring] !== undefined) color = frameColors[frameColoring];
            else if (frameColors[frameMaterial]) color = frameColors[frameMaterial];
        } else {
            // Если "без окраса", берем цвет материала
            if (frameColors[frameMaterial]) color = frameColors[frameMaterial];
        }

        // Генерируем текстуры (Color + Normal)
        const maps = this.createProceduralWoodMaps(color);
    ...
    createRoofCovering(length, width, height, roofHeight, roofingMaterial, frontExt, backExt, roofType, roofColor) {
        if (this.roofSystem) {
            this.roofSystem.update({
                ...
                roofColor: roofColor,
                overhang: 0.5, // Standard overhang
