...
(function() {
    ...
    async function initApp() {
        try {
            ...
            // 7. Создаем глобальные функции для совместимости
            window.makeOrder = function() {
                presenter.onOrderClicked(presenter.getCurrentData());
            };
            
            window.saveCalculation = function() {
                presenter.onSaveClicked(model.calculateCost());
            };
            
            window.exportToPDF = async function() {
                await presenter.exportToPDF();
            };
            
            window.loadSavedCalculation = function(calcId) {
                presenter.loadSavedCalculation(calcId);
            };
            
            window.deleteSavedCalculation = function(calcId) {
                presenter.deleteSavedCalculation(calcId);
            };
            
            window.updateHistoryPanel = function() {
                if (presenter.storageManager) {
                    const savedCalculations = presenter.storageManager.getAllCalculations();
                    console.log('Сохраненные расчеты:', savedCalculations.length);
                    // TODO: Обновление панели истории в UI
                }
            };
            
            console.log('=== Приложение готово к работе ===');
            console.log('Доступ через window.CanopyApp');
            console.log('Доступны глобальные функции: makeOrder(), saveCalculation(), exportToPDF()');
